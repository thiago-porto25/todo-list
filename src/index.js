...
const init = (function () {
  ...
  const renderCustomAppPage = () => {
    ...
    if (userObj.projects[0] === undefined) {
      ...
      if (userObj.projects[0].todos[0] === undefined)
  ...
  const displayNewProject = (id) => {
    ...
    if (project.todos[0] === undefined) mainDomHandler.renderEmptyTodosText()
      ...
      project.todos.forEach((todo) => {
        const todoListItem = mainDomHandler.createTodosListItem(
          todo.title,
...
const eventsHandler = (function () {
    ...
    checkboxes.forEach((checkbox) =>
      checkbox.addEventListener('click', () => {
        const currentProject =
          variablesForControl.setCurrentlyDisplayedProject()
        const currentProjectId =
          currentProject.getAttribute('data-deleteproject')
        ...
        if (checkbox.checked === true) {
          ...
          userObj.projects[currentProjectId].todos[
            TaskListItemId
          ].isDone = false
        }
        localStorageHandler.update()
  ...
  const addListenerTaskInfoOnStart = () => {
    ...
    editBtns.forEach((editBtn) =>
      editBtn.addEventListener('click', (e) => {
        ...
        titleInput.value =
          userObj.projects[projectId].todos[currentTaskId].title
        dateInput.value =
          userObj.projects[projectId].todos[currentTaskId].dueDate
        notesInput.value =
          userObj.projects[projectId].todos[currentTaskId].notes

        const form = document.querySelector('.taskInfoFormOnStart')
        form.addEventListener('submit', prevent.Refresh)
        form.addEventListener('submit', () => {
          userObj.projects[projectId].todos[currentTaskId].title =
            document.querySelector('.infoTitleOnStart').value
          userObj.projects[projectId].todos[currentTaskId].dueDate =
            document.querySelector('.infoDateOnStart').value
          userObj.projects[projectId].todos[currentTaskId].notes =
            document.querySelector('.notesInfoOnStart').value

          modalDomHandler.removeTaskInfoModalOnStart()

          changesHandler.changeTaskTitle(
            currentTaskId,
            userObj.projects[projectId].todos[currentTaskId].title
          )
  ...
  const addListenerTaskInfoCreated = (taskId) => {
    ...
    editBtn.addEventListener('click', (e) => {
      ...
      form.addEventListener('submit', () => {
        userObj.projects[projectId].todos[currentTaskId].title =
          document.querySelector('.infoTitleCreated').value
        userObj.projects[projectId].todos[currentTaskId].dueDate =
          document.querySelector('.infoDateCreated').value
        userObj.projects[projectId].todos[currentTaskId].notes =
          document.querySelector('.notesInfoCreated').value
  ...
  const addListenerNewTask = () => {
    ...
    newTodoButton.addEventListener('click', () => {
      if (userObj.projects[0] === undefined) {
        ...
        setTimeout(() => {
          modalDomHandler.removeNoProjectsAlertModal()
        }, 3000)
      } else {
    ...
    projects.forEach((project) => {
      ...
      deleteButton.addEventListener('click', (e) => {
        ...
        const confirmDeleteButton = document.querySelector(
          '#finalDeleteProjectButtonOnStart'
        )
    ...
    projectList.forEach((project) => {
      project.addEventListener('click', (e) => {
        ...
        allProjects.forEach((projectItem) =>
          projectItem.setAttribute('class', 'projectsListItem')
        )
  ...
  const addListenerToDeleteCreatedProject = (elem) => {
    ...
    deleteButton.addEventListener('click', (e) => {
      ...
      const confirmDeleteButton = document.querySelector(
        '#finalDeleteProjectButtonCreated'
      )
  ...
  const addListenerNewProject = () => {
    ...
    newProjectButton.addEventListener(
      'click',
      modalDomHandler.displayNewProjectModal
    )
